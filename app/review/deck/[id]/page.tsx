...
export default function DeckReviewPage({ params }: Props) {
    const { id } = use(params);
    ...
    useEffect(() => {
        async function fetchCards() {
            try {
                ...
                const cardsRes = await fetch(`/api/review/due?deck_id=${id}&count_only=false`);
    ...
    if (cards.length === 0) {
        return (
            <div className="p-8 max-w-2xl mx-auto text-center py-16">
                ...
                <p className="text-muted-foreground text-sm mb-6">No cards due for this deck.</p>
                <Link href="/review">
