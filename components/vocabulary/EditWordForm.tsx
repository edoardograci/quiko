...
interface EditWordFormProps {
    ...
    initialExampleKo?: string;
    initialExampleEn?: string;
    onSuccess: (wordId: number) => void;
    onCancel: () => void;
}

export function EditWordForm({ initialWord, initialDefinitions = [], initialExampleKo = '', initialExampleEn = '', onSuccess, onCancel }: EditWordFormProps) {
    const { lang, t } = useLang();
    const [hangul, setHangul] = useState(initialWord.hangul || '');
    const [hanja, setHanja] = useState(initialWord.hanja || '');
    const [pos, setPos] = useState(initialWord.pos || '');
    const [level, setLevel] = useState(initialWord.level || 'custom');
    const [stem, setStem] = useState(initialWord.stem || '');
    ...
    const [exampleKo, setExampleKo] = useState(initialExampleKo);
    const [exampleEn, setExampleEn] = useState(initialExampleEn);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch(`/api/words/${initialWord.id}`, {
                method: 'PUT',
            ...
            const data = await res.json();
            toast.success(t({ ko: `"${hangul}" 단어가 수정되었습니다!`, en: `Word "${hangul}" updated!` }));
            onSuccess(initialWord.id);
        } catch (e: unknown) {
            const msg = e instanceof Error ? e.message : t({ ko: '단어 수정 실패', en: 'Failed to update word' });
            toast.error(msg);
        } finally {
            setSubmitting(false);
        }
